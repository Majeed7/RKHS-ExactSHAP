...
plt.rcParams.update({
    'font.size': 13,     # Set default font size
...
excel_file_name = "sv_normalized_svObs"
...
if not plot_only:
    ...
    for d in ds:
        ...
        for i, x in enumerate(X_tbx):
            ...
            explainer = RBFLocalExplainer(model, value_function='baseline')
            ...
            sheet_rbf.append([i] + list(shapley_values))
...
for idx, d in enumerate(ds):
    # Load results
    excel_file = os.path.join(results_dir, f"d{d}_" + excel_file_name + ".xlsx")
...
for idx, d in enumerate(ds):
    # Load results
    # excel_file = os.path.join(results_dir, f"{excel_file}.xlsx")
    workbook = load_workbook(excel_file)
...
plt.tight_layout()
plt.show()
plt.savefig(f"{results_dir}/time.png", dpi=500, format='png', bbox_inches='tight')

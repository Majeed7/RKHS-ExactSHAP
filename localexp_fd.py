...
for idx, d in enumerate(ds):
    ...
    rbf_shapley_values = np.array([row[1:] for row in sheet_rbf.iter_rows(min_row=1, max_row=n_instances, values_only=True)])
    ...
    for sheet_name in workbook.sheetnames:
        ...
        shapley_regression_values = np.array([row[1:] for row in sheet.iter_rows(min_row=1, max_row=n_instances, values_only=True)])

        errors = np.linalg.norm(shapley_regression_values - rbf_shapley_values, axis=1, ord=1) / np.sum(rbf_shapley_values, axis=1)
